...
(transformer_bidir_on igbt ckpt) 提供 PCPF + Ip。粗网格 (nx=65) FreeGSNKE 求解, 收集收敛
...
合并 + 画图: python -m paper_igbt.aggregate_freegsnke_testset。

用法 (DCU 计算节点):
  python -m paper_igbt.freegsnke_testset_eval --n-shots 999999 --frames-per-phase 3 \
...
def main() -> int:
    ...
    from paper_igbt.freegsnke_infer import infer_one_shot_best
    from paper_igbt.model import BEST_CKPT

    ap = argparse.ArgumentParser(description=__doc__, formatter_class=argparse.RawDescriptionHelpFormatter)
    ap.add_argument("--ckpt", default=str(BEST_CKPT))
    ap.add_argument("--test-shots", default="meta/split_by_order_igbt/test_shots.txt")
    ...
    ap.add_argument("--out-dir", default="results/paper_igbt/freegsnke_testset")
    args = ap.parse_args()
    ...
    print(f"[shard {args.shard_id}/{args.n_shards}] {len(shots)} shots "
          ...
          f"n_proc={args.n_proc}, nx={args.nx}", flush=True)
    ...
    for si, shot in enumerate(shots, 1):
        try:
            ...
            inf = infer_one_shot_best(shot, args.ckpt)
        except Exception as e:
        ...
        pred_A = np.asarray(inf["pred_A"], float); ip_A = np.asarray(inf["Ip_A"], float).ravel()
        ...
        for ph_idx, ph in enumerate(PHASE_NAMES):
            ...
            for k in ks_all[::step][:args.frames_per_phase]:
                ...
                try:
                    export_slice_npz(bundle, k, sp, pcpf12_override=pred_A[k_bc],
