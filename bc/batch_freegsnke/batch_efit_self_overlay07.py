...
阶段名与 ``bc.data.phases`` 一致，由 precursor 的 ``ATIME`` + ``PCRL01`` 划分 ramp_up / flat_top / ramp_down，并传入 ``--out-phase`` 用于图题。

::

    python -m bc.batch_freegsnke.batch_efit_self_overlay07 --seed 2026 --shot 105653 \\
...
示例（代表时刻 + 拼图）::
    ...
    python -m bc.batch_freegsnke.batch_efit_self_overlay07 --seed 2026 --shot 105653 --stitch

    # 只拼图（假定各目录已算过 forward）
    python -m bc.batch_freegsnke.batch_efit_self_overlay07 --seed 2026 --shot 105653 --stitch-only
"""
