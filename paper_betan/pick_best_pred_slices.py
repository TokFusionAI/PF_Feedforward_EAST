#!/usr/bin/env python3
"""为 paper_betan 的 pred_wall_dist 图，按 s44 预测误差挑出每阶段最优时间片。

背景: paper_betan/freegsnke_whole_shot.py 现在用 ``pick_representative_steps``
...
by_t_efit 指标排序的 top-N，并强制包含当前 paper_betan 选中帧与 run1 最优帧。
...
def main() -> int:
    ...
    ap.add_argument("--whole-shot-root", default="results/paper_betan/freegsnke_whole_shot")
    ...
    # ── 候选 k_efit: run1 全局 top + 当前 paper_betan 选中 + run1 各阶段最优 ──
    ...
    # 当前 paper_betan 选中帧 (manifest middle picks)
