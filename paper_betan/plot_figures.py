...
def load_records(npz_path: Path) -> list[dict]:
    ...
    shots, T = d["shots"], d["T"]
    # 把压缩 npz 的成员提到循环外: 否则每次 d["..."] 都会重新解压整个数组(慢 ~1000×)。
...
def plot_loss_curves(tb_root: Path, out: Path, tag: str = "transformer_bidir_on_betan_s44"):
    ...
    # 8-seed val 带
    val_curves = []
    for sub in sorted(tb_root.glob("transformer_bidir_on_betan_s*")):
...
def main() -> int:
    ...
    ap.add_argument("--tb-root", default="results/betan_ablation/transformer_bidir_on/tb")
