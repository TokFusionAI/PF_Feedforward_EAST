...
def _render_subplot(ax, eq, snap, *, color, edge_color, legend_label):
    ...
    ax.set_xlabel(r"$\mathbf{R}$ (m)", fontsize=15, fontweight="bold")
    ax.set_ylabel(r"$\mathbf{Z}$ (m)", fontsize=15, fontweight="bold")
    ax.set_aspect("equal")
...
def main() -> int:
    ...
    legend_label = r"$\mathbf{FreeGSNKE}$ $\mathbf{I_{Pred}}$ $\mathbf{(R_8,Z_8)}$"
