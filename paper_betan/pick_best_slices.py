...
前置: ``paper_betan/export_all_slices.py`` (DCU 推理) 已导出全部 EFIT 帧 bc_pred 切片到
...
示例 (登录节点)::
    ...
    python paper_betan/pick_best_slices.py --shot 134925
"""
...
from paper_betan.plot_pred_wall_dist import _forward_solve_slice  # noqa: E402
...
def main() -> int:
    ...
    # 平顶段 best 片只从中段选 (避开贴近 ramp_up/ramp_down 的边缘); 其余相不变.
    FLAT_EDGE_FRAC = 0.25
    _flat_ph = next((p for p in PHASE_NAMES if "flat" in p.lower()), None)
    _flat_ks = [k for k in range(k0, k1)
                if _discharge_phase_for_precursor_k(phase_ids, k) == _flat_ph] if _flat_ph else []
    if len(_flat_ks) >= 4:
        _edge = max(1, int(len(_flat_ks) * FLAT_EDGE_FRAC))
        flat_middle = set(_flat_ks[_edge:len(_flat_ks) - _edge])
    else:
        flat_middle = set(_flat_ks)
    print(f"[flat_top] n={len(_flat_ks)} 中段候选 n={len(flat_middle)} "
          f"(去首尾各 {FLAT_EDGE_FRAC:.0%})", flush=True)
    ...
    for k in range(k0, k1):
        ...
        cur = best[ph]
        # flat_top 只接受中段候选 (避开贴近 ramp 的边缘); 其余相照旧
        _eligible = (ph != _flat_ph) or (k in flat_middle)
        if _eligible and (cur is None or rmsep < cur["lcfs_rmsep_m"]):
    ...
    manifest: dict[str, Any] = {
        ...
        "metric": "bc_pred 8-control-point Euclidean RMSE via shape_params (best per phase; paper_betan)",
