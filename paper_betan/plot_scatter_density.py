...
每风格两张图:
  主图 3×4 网格（12 PF 通道，统一量程，红虚线 y=x，R²/RMSE 角标，共享 log count 色条）
  附图   全通道池化 jointplot（中心密度 + 顶部/右侧边缘直方图；默认 per-channel z-score 池化）

角标只用 R² + RMSE（不用 Pearson r）。

数据加载/指标/通道名来自 paper_betan.records（与评测脚本同口径）。

读:  results/betan_ablation/transformer_bidir_on/transformer_bidir_on_betan_s44/per_shot_preds.npz
写:  results/paper_betan/figures/per_channel_scatter_{kde,hist,hex}.png
     results/paper_betan/figures/pooled_scatter_joint_{kde,hist,hex}.png
"""
...
from paper_betan.records import PF_NAMES, _metrics_1d, load_records
...
def _rmse(preds, truths):
    """均方根误差 (与 _metrics_1d 同口径: 全量 mask 上的 pred-truth)。"""
    res = preds - truths
    return float(np.sqrt(np.mean(res ** 2)))


def drop_pf6_neg_r2(records, pf6: int = PF_NAMES.index("PF6"),
                    out_csv: Path | None = None):
    """剔除 per-shot PF6 R²<0 的炮，返回保留的 records。

    PF6 R²<0 集中出现在高电流段系统性欠预测的炮（非破裂/非短炮）；剔除后角标 R² 会上升。
    out_csv 非空时，把被剔除炮号 + 其 PF6 R² 落盘备查。
    """
    keep, dropped = [], []
    for r in records:
        m = r["action_mask"][:, pf6]
        ...
        t = r["target_kA"][:, pf6][m]
        p = r["pred_kA"][:, pf6][m]
        r2 = _metrics_1d(p, t)[0]
        if r2 < 0:
            dropped.append((r, r2))
        else:
            keep.append(r)
    dropped_rows = [(r["shot"], r2, int(r["action_mask"][:, pf6].sum())) for r, r2 in dropped]
    print(f"drop PF6 R²<0: removed {len(dropped_rows)} / {len(records)} shots, kept {len(keep)}")
    if out_csv is not None and dropped_rows:
        import csv
        out_csv.parent.mkdir(parents=True, exist_ok=True)
        with out_csv.open("w", newline="") as f:
            w = csv.writer(f)
            w.writerow(["shot", "PF6_R2", "PF6_n_valid"])
            w.writerows(sorted(dropped_rows, key=lambda x: x[1]))
        print(f"  dropped shot list -> {out_csv}")
    return keep
...
def _print_r2_table(records):
    """打印 per-channel R²/RMSE，便于核对（图上角标的来源）。"""
    r2s = []
    for ch in range(12):
        t, p = _gather_channels(records, ch)
        r2 = _metrics_1d(p, t)[0]
        rmse = _rmse(p, t)
        r2s.append(r2)
        print(f"  {PF_NAMES[ch]:6s} R²={r2:.3f}  RMSE={rmse:.3f} kA")
...
def _prep_grid(records, cap: int = 1_000_000):
    ...
    for ch in range(12):
        ...
        r2 = _metrics_1d(preds, truths)[0]
        rmse = _rmse(preds, truths)
        ...
        chans.append((truths, preds, r2, rmse))
    pad = 0.02 * (ghi - glo)
...
def render_grid(chans, lo, hi, out, gridsize: int, style: str, sigma: float):
    ...
    for ch in range(12):
        ...
        ax.grid(alpha=0.25)
        ax.text(0.03, 0.97, f"$R^2$={r2:.3f}\nRMSE={rmse:.2f} kA", transform=ax.transAxes,
...
def _prep_joint(records, normalize: bool, cap: int = 3_000_000):
    ...
    if preds.size > cap:
        ...
        tp, pp = truths, preds
    return tp, pp, r2, rmse, unit, lo, hi, normalize


def render_joint(prep, out, gridsize: int, style: str, sigma: float):
    tp, pp, r2, rmse, unit, lo, hi, normalize = prep
    ...
    ax_main.text(0.03, 0.97, f"$R^2$={r2:.3f}\nRMSE={rmse:.2f} {unit}", transform=ax_main.transAxes,
...
def main() -> int:
    ...
    ap.add_argument("--preds",
                    default="results/betan_ablation/transformer_bidir_on/transformer_bidir_on_betan_s44/per_shot_preds.npz")
    ...
    ap.add_argument("--drop-pf6-neg-r2", action="store_true",
                    help="剔除 per-shot PF6 R²<0 的炮后再画(高电流欠预测失效模式)；"
                         "剔除炮号列表写到 <out-dir>/_pf6_neg_r2_shots.csv。默认关。")
    args = ap.parse_args()
    ...
    if args.drop_pf6_neg_r2:
        recs = drop_pf6_neg_r2(recs, out_csv=out / "_pf6_neg_r2_shots.csv")
