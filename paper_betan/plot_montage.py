#!/usr/bin/env python3
"""Paper-quality montage: 2×3 grid (Pred / PCS × 3 phases) + Ip(t) strip.

Each subplot renders EFIT R8/Z8 (dashed) and FreeGSNKE R8/Z8 (solid) using
shape_params, exactly like the single-frame overlay (Figure 2).

Layout::

    Row 0 (Pred):  ramp_up   flat_top   ramp_down
    Row 1 (PCS):   ramp_up   flat_top   ramp_down
    Row 2 (Ip):    full-shot Ip(t) with phase markers

Output: PNG + PDF in ``results/bc_v1_notime/run1/paper_figures/``.

Usage::

    python scripts/plot_montage_paper.py --shot 134925

    python scripts/plot_montage_paper.py --shots-file shots.txt
"""
...
def _frame_info_from_manifest(manifest: dict[str, Any]) -> dict[str, dict[str, Any]]:
    """Return {phase: {"k": int, "time_s": float}} from a pcs_vs_pred manifest."""
...
def _forward_solve_slice(
    ...
    """Load slice NPZ, run forward solve, return (eq, snap) or None."""
...
def _render_subplot(
    ...
    *,
    ...
    """Render one subplot: EFIT R8/Z8 (dashed) + FreeGSNKE R8/Z8 (solid)."""
    ...
    if not active_rects:
        ...
        for nm, d in active_fb.items():
            ...
            ax.add_patch(rect)
            ax.text(cx, cy, nm, ha="center", va="center", fontsize=5, color="black", zorder=22,
    ...
    ax.set_aspect("equal")
    ax.grid(True, color="0.88", lw=0.3, alpha=1.0)
    ...
    if uniq:
        ax.legend(uniq.values(), uniq.keys(), loc="upper right",
                  frameon=True, fancybox=False, edgecolor="0.8", fontsize=7)
...
def _process_shot(*, shot, precursor_npz, whole_shot_root, out_dir, pf_xml, wall_xml):
    ...
    if not precursor_npz.is_file():
        print(f"[skip] shot={shot} no precursor {precursor_npz}", file=sys.stderr, flush=True)
        return 2
    ...
    if manifest is None:
        print(f"[skip] shot={shot} no manifest {manifest_path}", file=sys.stderr, flush=True)
        return 2
    picks = _frame_info_from_manifest(manifest)

    branches = [("bc_pred", "Prediction"), ("pcs", "PCS")]

    with plt.rc_context(PUB_RC):
        fig = plt.figure(figsize=(14.5, 13.5))
        gs = fig.add_gridspec(3, 3, height_ratios=[1, 1, 0.35], hspace=0.14, wspace=0.0,
    ...
    for row_idx, (branch, label) in enumerate(branches):
        for col_idx, phase in enumerate(PHASE_NAMES):
            ...
            k = pick["k"]
            t_s = pick["time_s"]
            # bc_pred slices use "bc" not "bc_pred" in filename
            file_tag = "bc" if branch == "bc_pred" else branch
            ...
            print(f"[forward] {label} {phase} k={k} t={t_s:.3f}s ...", flush=True)
    ...
    # align Ip strip x-limits to top-row subplots
    try:
        ax0 = axes_grid[0][0]
        ax2 = axes_grid[0][2]
        x_lo = ax0.get_position().x0
        x_hi = ax2.get_position().x1
        ip_pos = ax_ip.get_position()
        ax_ip.set_position([x_lo, ip_pos.y0, x_hi - x_lo, ip_pos.height])
    except Exception:
        pass

    # column headers: "Ramp Up $t$=X.XX s" etc.
    for col_idx, phase in enumerate(PHASE_NAMES):
        ...
        fig.text(0.5 * (pos.x0 + pos.x1), pos.y1 + 0.008,
                 phase_label[phase] + t_str,
                 ha="center", va="bottom", fontsize=11, fontweight="bold", color="black")

    # row labels: "Prediction", "PCS" (rotated)
    for row_idx, (_, label) in enumerate(branches):
        pos = axes_grid[row_idx][0].get_position()
        fig.text(pos.x0 - 0.05, 0.5 * (pos.y0 + pos.y1), label,
                 ha="right", va="center", fontsize=11, fontweight="bold", color="black", rotation=90)

    out_dir.mkdir(parents=True, exist_ok=True)
    png_path = out_dir / f"montage_paper_shot{shot}.png"
    pdf_path = out_dir / f"montage_paper_shot{shot}.pdf"
