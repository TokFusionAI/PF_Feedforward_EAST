#!/usr/bin/env bash
# cross-betan (chrono held-out betan) 消融【评估】: 对 18 个 best_val.pt, 用 paper_best_timepe 同款 predict + eval_test
# 在 betan test 集算 metrics_summary.json (ckpt 内 cfg 自动带 betan 划分 + betan norm_stats),
# 再 aggregate 成 6 行表 (RMSE/R²_med, mean±std over 3 seed)。需 1 张 DCU。
#
# 用法 (计算节点, 训练全部完成后):
#   bash scripts_ablation/run_betan_ablation_eval.sh
set -euo pipefail
cd "$(dirname "$0")/.."
...
$PY scripts_ablation/aggregate_betan_ablation.py
echo ">>> 结果: results/betan_ablation/table_betan.md (及 .csv)"
