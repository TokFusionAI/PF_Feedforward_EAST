#!/usr/bin/env bash
# 训练 IGBT-PWM 时序消融的【单个】(CFG, SEED) 对。
# 由 submit_igbt_ablation.sbatch --array=0-17 调用, 每个 array task 训一对,
# SLURM 按空闲节点持续 backfill -> 最快墙钟。模型 = 原随机划分消融配置 (仅 --override
# 改 划分/norm/out_root/seed 到 IGBT-PWM 时序划分)。
#   IGBT-PWM split: meta/split_by_order_igbt (shot>=117203, 2022-11 PS11/12 更新后,
#   train 17038 / val 2130 / test 2129, 严格时序无交叠)。
#   norm 用 19 维 notime (ablation d_state=19, 不可用 21 维版)。
# 输出 results/igbt_ablation/<CFG>/。断点续跑: best_val.pt 存在则跳过。
#
